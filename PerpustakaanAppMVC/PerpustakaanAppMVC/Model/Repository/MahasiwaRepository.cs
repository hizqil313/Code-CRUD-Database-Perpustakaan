...
{
    public class MahasiwaRepository
    {
        private SQLiteConnection _conn;

        public MahasiwaRepository(DbContext context)
        {
            _conn = context.Conn;
        }
 ...
 values (@npm, @nama, @angkatan)";
        ...
        public int Update(Mahasiswa mhs)
        {
            int result = 0;
            // deklarasi perintah SQL
            string sql = @"update into mahasiswa (npm, nama, angkatan)
 values (@npm, @nama, @angkatan)";
            // membuat objek command menggunakan blok using
            using (SQLiteCommand cmd = new SQLiteCommand(sql, _conn))
            {
        ...
        public int Delete(Mahasiswa mhs)
        {
            int result = 0;
            // deklarasi perintah SQL
            string sql = @"delete into mahasiswa where npm=@npm";
            // membuat objek command menggunakan blok using
            using (SQLiteCommand cmd = new SQLiteCommand(sql, _conn))
            {
                // mendaftarkan parameter dan mengeset nilainya
